...
Improve minimatch pattern matching performance by at least 1% (measured as operations per second) on the standard benchmark of matching 1000 file paths against the `**/*.js` glob pattern, repeated 1000 times.

## What you CAN modify

- `src/**/*.ts` — TypeScript source code files
...
- `.polyresearch-node.toml` — configuration
- `benchmark.cjs` — benchmark script
- `test/**/*` — test files
