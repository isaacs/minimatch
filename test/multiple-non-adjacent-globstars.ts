...
t.test('GHSA-7r86-cg39-jmmj', async t => {
  ...
  t.comment(inspect({ patha, pathb, pattern }))
  ...
  t.comment(inspect({ dura, durb, durc }))
...
t.test('tail handling 2', async t => {
  ...
  const nomatch = '.x/'
  t.equal(minimatch(match, pattern, { dot: true }), true)
  t.equal(minimatch(nomatch, pattern, { dot: true }), false, 'should no match', {
    file: nomatch,
    pattern,
  })
})
